...
struct ProgramEditorView: View {
    ...
    init(
        ...
        _exercises = State(
            initialValue: existingProgram?.exercises ?? []
        )

        _scheduledDate = State(
            initialValue: existingProgram?.scheduledDate ?? Date()
        )
    }
                        ...
                        ForEach(exercises) { exercise in
                            VStack(
                                ...
                                Text(exercise.exerciseName)
                                    .font(.headline)
                                Text(
                                    "Sets: \(exercise.sets) | Reps: \(exercise.reps) | Weight: \(String(format: "%.1f", exercise.weight)) lbs"
                                )
                                .font(.caption)
    ...
    private func addExercise() {
        let trimmedName =
            exerciseName.trimmingCharacters(
                in: .whitespaces
            )
        guard
            !trimmedName.isEmpty,
